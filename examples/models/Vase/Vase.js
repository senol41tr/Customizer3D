    ...
    return {
        ...
        data:
        {
            vase:
            {
                ...
                materials: 
                [
                    {url: root + 'textures/5.jpg', repeatX: 2, repeatY: 2},
                    {url: root + 'textures/1.jpg', repeatX: 4, repeatY: 4},
                    {url: root + 'textures/2.jpg', repeatX: 4, repeatY: 4},
                    {url: root + 'textures/3.jpg', repeatX: 4, repeatY: 4},
                    {url: root + 'textures/4.jpg', repeatX: 4, repeatY: 4},
                    {url: root + 'textures/6.jpg', repeatX: 3, repeatY: 3},
                    {url: root + 'textures/7.jpg', repeatX: 2, repeatY: 2}
                ]
            }
        }
    };
...
export async function init()
{
    ...
    // set mesh materials
    const soilMap = await new Texture({url: root + 'maps/Plant-Set-002-grass.jpg', preloader: this.preloader}).load();
    ...
    const leavesMap = await new Texture({url: root + 'maps/03.jpg', preloader: this.preloader}).load();
    const leavesAlphaMap = await new Texture({url: root + 'maps/03_Opacity.jpg', preloader: this.preloader}).load();
    const leavesNormalMap = await new Texture({url: root + 'maps/03_Normal.jpg', preloader: this.preloader}).load();
