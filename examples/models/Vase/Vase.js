...
import {Texture} from 'customizer3D_dir/three/loaders/Texture.js?c3d=102';
    ...
    return {
        ...
        data:
        {
            vase:
            {
                ...
                materials: 
                [
                    {url: root + 'textures/5.jpg?c3d=102', repeatX: 2, repeatY: 2},
                    {url: root + 'textures/1.jpg?c3d=102', repeatX: 4, repeatY: 4},
                    {url: root + 'textures/2.jpg?c3d=102', repeatX: 4, repeatY: 4},
                    {url: root + 'textures/3.jpg?c3d=102', repeatX: 4, repeatY: 4},
                    {url: root + 'textures/4.jpg?c3d=102', repeatX: 4, repeatY: 4},
                    {url: root + 'textures/6.jpg?c3d=102', repeatX: 3, repeatY: 3},
                    {url: root + 'textures/7.jpg?c3d=102', repeatX: 2, repeatY: 2}
                ]
            }
        }
    };
...
export async function init()
{
    ...
    // set mesh materials
    const soilMap = await new Texture({url: root + 'maps/Plant-Set-002-grass.jpg?c3d=102', preloader: this.preloader}).load();
    ...
    const leavesMap = await new Texture({url: root + 'maps/03.jpg?c3d=102', preloader: this.preloader}).load();
    const leavesAlphaMap = await new Texture({url: root + 'maps/03_Opacity.jpg?c3d=102', preloader: this.preloader}).load();
    const leavesNormalMap = await new Texture({url: root + 'maps/03_Normal.jpg?c3d=102', preloader: this.preloader}).load();
