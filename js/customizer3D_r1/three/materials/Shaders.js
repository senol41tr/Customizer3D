...
import {getMaxLayers} from 'customizer3D_dir/settings/GPUInfo.js?c3d=102';
...
export const getMainUniforms = (c3d) =>
{
    ...
    return {
        ...
        uBlendModes: {value: new Int32Array(MAX_LAYERS).fill(0)},
        uAlphas: {value: new Float32Array(MAX_LAYERS).fill(1.0)},
        uActiveLayerCount: {value: MAX_LAYERS},
        uRenderOrder: {value: new Int32Array(MAX_LAYERS).fill(MAX_LAYERS - 1)},
        uOffset: {value: Array.from({ length: MAX_LAYERS }, () => new THREE.Vector2(0, 0))},
        uZoom: {value: new Float32Array(MAX_LAYERS).fill(1.0)},
        uRotation: {value: new Float32Array(MAX_LAYERS).fill(0.0)},
        uAspect: {value: 1.0},

        uBrightness: {value: new Float32Array(MAX_LAYERS).fill(1.0)},
        uContrast: {value: new Float32Array(MAX_LAYERS).fill(1.0)},
        uHue: {value: new Float32Array(MAX_LAYERS).fill(0.0)},
        uSaturation: {value: new Float32Array(MAX_LAYERS).fill(1.0)},
        uSepia: {value: new Float32Array(MAX_LAYERS).fill(0.0)},
        uChromaticAmount: {value: Array.from({ length: MAX_LAYERS }, () => new THREE.Vector2(0, 0))},
        uInvert: {value: new Float32Array(MAX_LAYERS).fill(0.0)},
        uVignette: {value: new Float32Array(MAX_LAYERS).fill(0.0)}, // -1.0 (Black) to 1.0 (White)
        uGrainAmount: {value: new Float32Array(MAX_LAYERS).fill(0.0)},
        uTint: {value: Array.from({ length: MAX_LAYERS }, () => new THREE.Color(0x0)) },
        uTintAmount: {value: new Float32Array(MAX_LAYERS).fill(0.0)}
...
export const getMainUniforms = (c3d) =>
{
    ...
    return {
        tBase: {value: null},
        uLayerTextures: {value: null},
        uBlendModes: {value: new Uint8Array(MAX_LAYERS).fill(0)},
        uAlphas: {value: new Float16Array(MAX_LAYERS).fill(1.0)},
        uActiveLayerCount: {value: MAX_LAYERS},
        uRenderOrder: {value: new Uint8Array(MAX_LAYERS).fill(MAX_LAYERS - 1)},
        uOffset: {value: Array.from({ length: MAX_LAYERS }, () => new THREE.Vector2(0, 0))},
        uZoom: {value: new Float16Array(MAX_LAYERS).fill(1.0)},
        uRotation: {value: new Float16Array(MAX_LAYERS).fill(0.0)},
        uAspect: {value: 1.0},

        uBrightness: {value: new Float16Array(MAX_LAYERS).fill(1.0)},
        uContrast: {value: new Float16Array(MAX_LAYERS).fill(1.0)},
        uHue: {value: new Float16Array(MAX_LAYERS).fill(0.0)},
        uSaturation: {value: new Float16Array(MAX_LAYERS).fill(1.0)},
        uSepia: {value: new Float16Array(MAX_LAYERS).fill(0.0)},
        uChromaticAmount: {value: Array.from({ length: MAX_LAYERS }, () => new THREE.Vector2(0, 0))},
        uInvert: {value: new Float16Array(MAX_LAYERS).fill(0.0)},
        uVignette: {value: new Float16Array(MAX_LAYERS).fill(0.0)}, // -1.0 (Black) to 1.0 (White)
        uGrainAmount: {value: new Float16Array(MAX_LAYERS).fill(0.0)},
        uTint: {value: Array.from({ length: MAX_LAYERS }, () => new THREE.Color(0x0)) },
        uTintAmount: {value: new Float16Array(MAX_LAYERS).fill(0.0)},

        uIsGradient: {value: new Uint8Array(MAX_LAYERS).fill(0)},
        uGradientColors: { value: new Array(MAX_LAYERS * MAX_GRADIENT_COLORS).fill(0).map(() => new THREE.Color()) },
        uGradientCount: { value: new Uint8Array(MAX_LAYERS).fill(0) },
        uGradientIsRadial: { value: Array.from({ length: MAX_LAYERS }, () => false) },
        uGradientAngle: { value: new Float16Array(MAX_LAYERS).fill(0.0) },
        uGradientScale: { value: new Float16Array(MAX_LAYERS).fill(0.0) }
...
export const getMainFragmentShader = (c3d) =>
{
    ...
    return `
        ...
        uniform int uIsGradient[${MAX_LAYERS}];
        uniform vec3 uGradientColors[${MAX_LAYERS * MAX_GRADIENT_COLORS}];
        uniform int uGradientCount[${MAX_LAYERS}];
        uniform bool uGradientIsRadial[${MAX_LAYERS}];
        uniform float uGradientAngle[${MAX_LAYERS}];
        uniform float uGradientScale[${MAX_LAYERS}];
        ...
        vec3 getGradient(int i, float t) {
            int baseOffset = i * ${MAX_GRADIENT_COLORS};
            float stepSize = 1.0 / float(uGradientCount[i] - 1);
            int index = int(t / stepSize);
            float segmentProgress = (t - float(index) * stepSize) / stepSize;
            index = clamp(index, 0, uGradientCount[i] - 2);
            return mix(
                uGradientColors[baseOffset + index], 
                uGradientColors[baseOffset + index + 1], 
        ...
        void main()
        {
            ...
            for(int i = ${MAX_LAYERS}; i >= 0 ; i--)
            {
                ...
                if(lUv.x >= 0.0 && lUv.x <= 1.0 && lUv.y >= 0.0 && lUv.y <= 1.0)
                {
                    ...
                    if(uIsGradient[actualIdx] == 1)
                    {
                        float gradT;
                        if(uGradientIsRadial[actualIdx]) {
                            gradT = distance(vUv, vec2(0.5));
                        } else {
                            float rad = radians(uGradientAngle[actualIdx]);
                        ...
                        vec2 offset = vec2(float(actualIdx) * 12.34); 
                        float noise = snoise((vUv + offset) * uGradientScale[actualIdx]);
                        
                        float noiseT = noise * 0.5 + 0.5;
                        float t = mix(gradT, noiseT, 0.3); 
                        layerRGB = getGradient(actualIdx, t);
                        layerAlpha = uAlphas[actualIdx];
                    }
                    else
                    {
                        vec4 texSample = texture(uLayerTextures, vec3(lUv, float(actualIdx)));
                        ...
                        layerAlpha = texSample.a * uAlphas[actualIdx];
                    }
...
export const fragmentShader2 = `
    ...
    void main() {
        ...
        float originalAlpha = tex.a * uOpacity;

        // Boundary Check (Alpha Clipping)
        if(uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0) {
           originalAlpha = 0.0;
        }

        if(originalAlpha < 0.001) discard; // Kill the pixel immediately if it's transparent

        vec3 color = tex.rgb;

        // 2. PREPARE BASE COLOR with Chromatic Shift
