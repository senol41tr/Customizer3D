...
import {trimCanvas} from 'customizer3D_dir/utils/trimCanvas.js?c3d=104';
import {BlobtoUint8Array} from 'customizer3D_dir/utils/BlobtoUint8Array.js?c3d=104';
import {Base64ToUint8} from 'customizer3D_dir/utils/Base64ToUint8.js?c3d=104';
        ...
        for (let j = layers.length - 1; j >= 0; j--)
        {
          ...
          switch (layer.type)
          {
              ...
              case 'shape':
                  ...
                  let printDims = getPrintDims(this.c3d, layer.name, 300);
                  ...
                  const shapeLayerCanvas = this.c3d.shapeLayer.htmlEl.querySelector('canvas.preview');
                  const widthShape = Math.floor(printDims.width);
                  const heightShape = Math.floor(printDims.height);

                  this.c3d.shapeLayer.show(layer, widthShape, heightShape);

                  response = await new Promise(resolve => shapeLayerCanvas.toBlob(resolve, 'image/png', 1.0));
                  uint8Array = await BlobtoUint8Array(response);
                  image = pdf.embedPng(uint8Array);
